...
import memosHelper from '../utils/memosHelper';
import matter from 'gray-matter';

ipcMain.on('update-file', (event, { path, content }) => {
  memosHelper.updateFile(path, content);
});

ipcMain.on('change-folder', (event, newPath) => {
  memosHelper.changeWatchFolder(newPath);
});
...
ipcMain.handle('get-files', async (event, dirPath) => {
  const files = await memosHelper.getFilesInFolder(dirPath);
  return files;
});

ipcMain.handle('get-file', async (event, filePath) => {
  const content = await memosHelper.getFile(filePath).catch(() => null);
  return content;
});

ipcMain.on('get-config-file-path', (event) => {
  const userHomeDirectoryPath = app.getPath('home');
  const memoConfig = path.join(userHomeDirectoryPath, 'Memo', 'memos.json');
  event.returnValue = memoConfig;
});
